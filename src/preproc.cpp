...
InitCharTableBeta()
{
  ...
  if (verbose)
  {
    printf("Building chr_tablebeta[] for preprocessor. \n");
  }
  for (i = 0; i < 256; i++)
  {
    chr_tablebeta[i] = SPECIAL;
  }
  for (i = '0'; i <= '9'; i++)
  {
    chr_tablebeta[i] = DIGIT;
  }
  for (i = 'A'; i <= 'Z'; i++)
  {
    chr_tablebeta[i] = LETTER;
  }
  for (i = 'a'; i <= 'z'; i++)
  {
    chr_tablebeta[i] = LETTER;
            ...
            if (*DataPtr == 'I' || *DataPtr == 'i')
            {
              ...
              if (*DataPtr == 'N' || *DataPtr == 'n')
              {
                ...
                if (*DataPtr == 'E' || *DataPtr == 'e')
                {
                  ...
                  GetIdentifierBeta();
                  memcpy(
                      &IdentifierArray[NumIdentifiers - 1][0], tokenbeta, 40);
                  if (verbose)
                  {
                    printf(
                        "Found macro identifier %s\n",
                        IdentifierArray[NumIdentifiers - 1]);
                  }
                  GetIdentifierGamma();
                  memcpy(
                      &ExpansionArray[NumIdentifiers - 1][0], tokenbeta, 512);
                  if (verbose)
                  {
                    printf(
                  ...
                  if (verbose)
                  {
                    printf(
                        "params: %s%s%s%s\n",
                        ParamArray[NumIdentifiers - 1][0],
                        ParamArray[NumIdentifiers - 1][1],
                        ParamArray[NumIdentifiers - 1][2],
                        ParamArray[NumIdentifiers - 1][3]);
                  }
                  MagicNumber = DataPtr;
                }
              }
